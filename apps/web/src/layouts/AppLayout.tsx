...
export default function AppLayout() {
  ...
  return (
    <div className="flex h-[100dvh] overflow-hidden bg-slate-100 dark:bg-[#0f0f1a]">
      ...
      {/* ── Mobile bottom nav — floating pill with center FAB ── */}
      <div
        className="fixed bottom-0 inset-x-0 z-40 md:hidden flex justify-center pb-[env(safe-area-inset-bottom)]"
        style={{ paddingBottom: 'max(env(safe-area-inset-bottom), 12px)' }}
      >
        <nav
          aria-label="Main navigation"
          className="flex items-center mx-4 w-full max-w-sm rounded-2xl border border-black/[0.07] dark:border-white/[0.08] bg-white/90 dark:bg-[#0d0d1a]/95 backdrop-blur-2xl shadow-xl shadow-black/[0.12] px-2"
        >
          {/* Left items */}
          ...
          <div className="flex-1 flex justify-center items-center py-1.5">
            <button
              ...
              className={[
                ...
                'focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-[#0d0d1a]',
                // Raised above the bar
                '-translate-y-4 border-4 border-white dark:border-[#0d0d1a]',
              ].join(' ')}
            >
      ...
      <button
        ...
        aria-label="Add expense"
        className={[
          'fixed z-40 hidden md:flex items-center gap-2 rounded-2xl bg-indigo-600 text-white shadow-lg shadow-indigo-500/30 px-4 py-3',
          'hover:bg-indigo-500 active:scale-95 transition-all duration-150 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2',
          'bottom-8 right-8 text-sm font-semibold',
        ].join(' ')}
      >
